...
thres = 90
if origin_sz < 600 ** 2: thres = 60
elif origin_sz < 800 * 2: thres = 0
elif origin_sz < 900 * 2: thres = 80
...
def segment_by_angle_kmeans(lines, k=2, **kwargs):
    ...
    avg1, avg2 = np.average([abs(np.sin(segmented[0][i][0][1])) for i in range(len(segmented[0]))]), np.average([abs(np.sin(segmented[1][i][0][1])) for i in range(len(segmented[1]))])
    # print(avg1, avg2)
    if avg1 < avg2:
        segmented = list(reversed(segmented))
    return segmented
...
def intersection(line1, line2):
    ...
    if abs(np.linalg.det(A)) <= 10 ** -4:
        if np.abs(rho1 - rho2) <= 40: return (0, 0)
        else: return (-10 ** 9, 0)
...
for pt1, pt2, pt3, pt4 in points:
    ...
    for a, b in arr:
        ...
        rate = 0.85
        if origin_sz < 600 ** 2: rate = 0.65
        elif origin_sz < 700 ** 2: rate = 0.75
        elif origin_sz < 1000 ** 2: rate = 0.8
...
for cnt, arr, inter in squares:
    ...
    if cnt >= 3:
        ...
        for a, b in arr:
            for i in [a, b]:
                lim = 0.65
                if origin_sz < 600 ** 2: lim = 0.65
                elif origin_sz < 800 ** 2: lim = 0.65
...
if ansX < 10: ansX, ansY = 8, 8
else: ansX, ansY = 10, 10
