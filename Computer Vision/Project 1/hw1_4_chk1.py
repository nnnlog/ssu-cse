...
if ansX < 10:
    ...
    ansX, ansY = 10, 10
print("refined board size: " + str(ansX) + " * " + str(ansY))
...
# Projection!
res, mat = convert_image_to_square(img, [pt1, pt2, pt3, pt4])

res = cv.bilateralFilter(res, -1, 32, 16)

dxdy = cv.Canny(res, 50, 250)
find_lines = cv.HoughLines(dxdy, 1, np.pi / 180, 50, None, 0, 0)

res_edge = np.zeros(res.shape, dtype=np.uint8)
...
segmented = segment_by_angle_kmeans(find_lines)
...
for segment in segmented:
    ...
    for i in range(len(segment)):
        ...
        for j in range(i + 1, len(segment)):
            if vis[j]: continue
            x, y = intersection(segment[i], segment[j], rohThres=(res.shape[0] // ansX - 30))
            if get_dist(res.shape[0] / 2, res.shape[1] / 2, x, y) <= max(*res.shape) * 4:
                vis[j] = 1
            # if -150 <= x <= 1150 and -150 <= y <= 1150: vis[j] = 1
            # else: tmp.append((x, y))
        curr.append(segment[i])
...
for i in range(len(X)):
    for j in range(len(X[i]) - 1):
        ...
        if inv_X[pt3][0] != inv_X[pt4][0]:
            ...
            continue
        # print(intersections[pt1], intersections[pt2], intersections[pt3], intersections[pt4])
        line1 = list(final_segmented[0][inv_X[pt1][0]])
        line2 = list(final_segmented[0][inv_X[pt4][0]])
        line3 = list(final_segmented[1][inv_Y[pt1][0]])
        line4 = list(final_segmented[1][inv_Y[pt2][0]])
        ...
        #         x2 = int(x0 - length * (-b))
        #         y2 = int(y0 - length * (a))
        #         cv.line(res_edge, (x1, y1), (x2, y2), (0, 0, 255), 1, cv.LINE_8)
        ...
        line3[0] += delta
        line4[0] -= delta
        pt1 = intersection(line1, line4)
        pt2 = intersection(line1, line3)
        pt3 = intersection(line3, line2)
        pt4 = intersection(line2, line4)
...
colors = []
for pt1, pt2, pt3, pt4 in points:
    pt1, pt2, pt3, pt4 = list(pt1), list(pt2), list(pt3), list(pt4)
    curr, mat = convert_image_to_square(res, [pt1, pt2, pt3, pt4])
    curr = cv.bilateralFilter(curr, 0, 3, 3)
    # curr = cv.medianBlur(curr, 5)


    # code from https://docs.opencv.org/3.4/d4/d70/tutorial_hough_circle.html
    gray = cv.cvtColor(curr, cv.COLOR_BGR2GRAY)
    gray = cv.Canny(gray, 20, 100)

    tmp = np.zeros(shape=curr.shape, dtype=np.uint8)
    ...
    if circles is not None:
        mask = np.zeros(shape=curr.shape, dtype=np.uint8)
        ...
        for i in circles[0, :]:
            ...
            tmp = np.zeros(shape=curr.shape, dtype=np.uint8)
            cv.circle(tmp, center, radius, (255, 255, 255), cv.FILLED)
            if np.count_nonzero(cv.bitwise_and(curr, tmp)) <= curr.shape[0] * curr.shape[1] // 8: continue
            cv.circle(mask, center, radius, (255, 255, 255), cv.FILLED)
            ok = 1
            tmp = np.copy(curr)
            cv.circle(tmp, center, radius, (255, 0, 255))
            ...
            cv.waitKey(0)
        if not ok: continue
        hsl = cv.cvtColor(curr, cv.COLOR_BGR2HLS)
